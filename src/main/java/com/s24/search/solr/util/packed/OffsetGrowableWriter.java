 ...
 * cardinality of values stored in the growable writer, which reduces ram usage.
 * 
...
public class OffsetGrowableWriter extends GrowableWriter {
   ...
   public OffsetGrowableWriter(int startBitsPerValue, int valueCount, float acceptableOverheadRatio, long minimalValue) {
    ...
    * stored are fixed to meet the new minimium value.
    */
   protected void ensureMinimumBoundary(long value) {
      ...
      if (value < minimumValue) {
         ...
         try {
            for (DocIdSetIterator ids = valuesFilled.iterator(); ids.nextDoc() != DocIdSetIterator.NO_MORE_DOCS;) {
